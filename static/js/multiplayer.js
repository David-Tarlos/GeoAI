...
let mpGuessed      = false;
let mpTimerInt     = null;
let mpResultAutoMs = 6000;   // ms to show results before "Weiter" button appears
  ...
  if (!ov) {
    ...
    ov.innerHTML = `
      <div id="mp-lobby-box">
        ...
        <!-- Zeit pro Runde — nur für Host sichtbar -->
        <div id="mlo-time-setting" style="display:none">
          <div class="mlo-set-label">Zeit pro Runde: <span id="mlo-time-val">20s</span></div>
          <input type="range" id="mlo-time-inp" min="5" max="60" step="5" value="20"
                 oninput="document.getElementById('mlo-time-val').textContent=this.value+'s'">
        </div>
...
function onMpJoinOk(data) {
  ...
  // ── URL fix ──
  const correctUrl = `/room/${data.code}?name=${encodeURIComponent(mpName)}`;
  if (window.location.pathname + window.location.search !== correctUrl) {
    history.replaceState({ roomCode: data.code }, '', correctUrl);
  }
  ...
  if (state === 'lobby') {
    ...
    if (mpIsHost) {
      ...
      const timeSt = document.getElementById('mlo-time-setting');
      if (btn)    btn.style.display    = 'block';
      if (st)     st.textContent       = 'Alle bereit? Starte das Spiel!';
      if (timeSt) timeSt.style.display = 'block';
    }
  }
}
...
function mpStartGame() {
  if (!mpIsHost) return;
  const timeInp = document.getElementById('mlo-time-inp');
  const timeout = timeInp ? parseInt(timeInp.value) : 20;
  mpSocket.emit('start_game', { code: ROOM_CODE, timeout });
}
...
function showMpResultPanel(scores, ai_score, ai_dist, is_last) {
  ...
  const rows = allEntries.map((s, i) => {
    const badge   = s.isAI ? '🤖' : '●';
    const distTxt = s.dist != null
      ? `<span class="mpr-dist">${s.dist.toLocaleString()} km</span>` : '';
  ...
  // "Weiter" button hidden initially; revealed after mpResultAutoMs
  ov.innerHTML = `
    <div id="mp-result-modal">
      <div class="mpr-title">${is_last ? '🏆 Endstand' : '📍 Runden-Ergebnis'}</div>
      <div class="mpr-list">${rows}</div>
      <div class="mpr-btns">
        <div class="mpr-wait" id="mpr-wait-msg">
          ${mpIsHost ? 'Ergebnisse werden angezeigt…' : 'Warte auf Host…'}
        </div>
        ${mpIsHost
          ? `<button class="mpr-btn" id="mpr-next-btn" onclick="mpNext()" style="display:none">
               ${is_last ? 'Endstand anzeigen →' : 'Nächste Runde →'}
             </button>
             <div class="mpr-countdown" id="mpr-countdown"></div>`
          : ''}
      </div>
    </div>`;
  ov.classList.add('show');

  // After delay: hide wait message, show Weiter button (host only)
  if (mpIsHost) {
    let remaining = Math.ceil(mpResultAutoMs / 1000);
    const cdEl   = document.getElementById('mpr-countdown');
    const btnEl  = document.getElementById('mpr-next-btn');
    const waitEl = document.getElementById('mpr-wait-msg');

    if (cdEl) cdEl.textContent = `Weiter in ${remaining}s…`;
...
function injectMpStyles() {
  ...
  style.textContent = `
    ...
    .mlo-set-label { font-size:.65rem; color:var(--muted); }
    #mlo-time-inp  { width:100%; accent-color:var(--green); cursor:pointer; }
    ...
    /* ── Round result overlay: no blur, anchored near top ── */
    #mp-result-overlay {
    ...
    #mp-result-modal {
      ...
      animation:mprSlideIn .3s cubic-bezier(.34,1.3,.64,1);
    ...
    .mpr-row {
      ...
      font-size:.72rem; background:rgba(255,255,255,.03);
    ...
    .mpr-btns  { margin-top:4px; display:flex; flex-direction:column; gap:6px; }
    .mpr-btn {
    ...
    .mpr-wait      { text-align:center; font-size:.65rem; color:var(--muted); padding:4px 0; }
    .mpr-countdown { text-align:center; font-size:.6rem; color:var(--muted); }
    ...
    #mp-timer {
      ...
      color:var(--muted); min-width:28px; text-align:center; transition:color .3s;
    }
    ...
    .mp-strip-player {
      ...
      border-radius:12px; font-size:.6rem;
      background:rgba(255,255,255,.03); border:1px solid currentColor; white-space:nowrap;
    }
